...
```
                   moodle-tool-guide.html  (Source of Truth)
...
| Komponente | Zweck |
|---|---|
| `moodle-tool-guide.html` | Source of Truth — gesamte App (React + i18n + Daten + Styles + Logos als Data-URIs). |
...
1. **Editor** ändert `moodle-tool-guide.html`.
...
**Überblick**
Source ist `moodle-tool-guide.html`. Die zwei Sync-Skripte sind die einzigen erlaubten Wege, die Plugin-JS-Dateien zu aktualisieren — manuelle Edits an `moodle-plugin/local/toolguide/amd/src/toolguide.js` oder `wordpress-plugin/eledia-toolguide/assets/js/toolguide.js` würden beim nächsten Sync verloren gehen.
