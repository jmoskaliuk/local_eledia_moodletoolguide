...
out += '    return;\n'
out += '  }\n'
out += '  const __elediaToolguideEnsureReact = () => {\n'
out += '    const wpElement = window.wp && window.wp.element ? window.wp.element : null;\n'
out += '    window.React = window.React || wpElement;\n'
out += '    if (!window.ReactDOM && wpElement) {\n'
out += '      window.ReactDOM = {\n'
out += '        render(element, container) {\n'
out += '          if (typeof wpElement.render === "function") {\n'
out += '            return wpElement.render(element, container);\n'
out += '          }\n'
out += '          if (typeof wpElement.createRoot === "function") {\n'
out += '            return wpElement.createRoot(container).render(element);\n'
out += '          }\n'
out += '        }\n'
out += '      };\n'
out += '    }\n'
out += '    return !!(window.React && window.ReactDOM && typeof window.ReactDOM.render === "function");\n'
out += '  };\n'
out += '  const __elediaToolguideStart = () => {\n'
...
out += app
out += '\n  };\n'
out += '  const __elediaToolguideBoot = attempt => {\n'
out += '    if (__elediaToolguideEnsureReact()) {\n'
out += '      __elediaToolguideStart();\n'
out += '      return;\n'
out += '    }\n'
out += '    if (attempt < 60) {\n'
out += '      window.setTimeout(() => __elediaToolguideBoot(attempt + 1), 100);\n'
out += '      return;\n'
out += '    }\n'
out += '    console.error("[eledia-toolguide] React/ReactDOM not available after waiting");\n'
out += '  };\n'
out += '  __elediaToolguideBoot(0);\n'
out += '})();\n'
