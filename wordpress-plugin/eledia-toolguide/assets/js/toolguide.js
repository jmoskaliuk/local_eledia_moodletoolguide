...
// Thumbs icons for matrix cells: up / sideways / down — pure Lucide paths.
// The "orange" (sideways) state is the same thumbs-up path; ThumbIcon rotates
// the entire SVG 90° clockwise via CSS so the thumb points right cleanly,
// without any off-center SVG transform artifacts.
...
const THUMBS = {
  ...
  orange: `<g ${LUCIDE_STROKE}>${THUMBS_UP_PATH}</g>`,
...
function ThumbIcon({rating, size=22, title=""}) {
  ...
  const rotate = rating === "orange" ? "transform:rotate(90deg);" : "";
  return React.createElement("span", {
    title: title,
    role: "img",
    "aria-label": title,
    ...
    dangerouslySetInnerHTML: {__html:`<svg width="${size}" height="${size}" viewBox="0 0 24 24" style="pointer-events:none;${rotate}">${THUMBS[rating]||THUMBS.rot}</svg>`}
  });
}
